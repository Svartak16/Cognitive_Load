...
<<<<<<< HEAD
// ---------- GEMINI AI ANALYSIS ----------
// const GEMINI_API_KEY = "AIzaSyBPQ5Vjs-kLTXZp2F-2MlbpgdiR1jCs2tQ";
=======
...
<<<<<<< HEAD
        const response = await fetch(
            'http://localhost:8787/gemini',
            {
                ...
                body: JSON.stringify({
                    kind: 'vision',
                    prompt: `You are a visual analysis assistant.
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  // Run ML model and Safe Browsing at the same time
  const [result, isGoogleFlagged] = await Promise.all([
    scanWithModel(tab.url),
    checkSafeBrowsing(tab.url),
  ]);

  if (result) {
    // If Google flagged it → override ML result with MALICIOUS
    if (isGoogleFlagged) {
      result.decision           = "MALICIOUS";
      result.recommended_action = "BLOCK";
      result.confidence         = 100;
      result.reason             = "Flagged by Google Safe Browsing";
    }

    chrome.storage.local.set({ lastScan: { url: tab.url, ...result } });
    chrome.tabs.sendMessage(tabId, { type: "PHISHING_RESULT", result });
  }
});
// ```
